...
namespace HtmlAgilityPack
{
	...
	public partial class HtmlNode
	{
		...
		public void WriteTo(XmlWriter writer)
		{
			switch (_nodetype)
			{
				...
				case HtmlNodeType.Element:
                    ...
                    string ns = null;
                    if (_attributes.Contains("xmlns"))
                        ns = _attributes["xmlns"].Value;
		...
		internal static void WriteAttributes(XmlWriter writer, HtmlNode node)
		{
			...
			{
                if (att.XmlName != "xmlns")
                {
                    if (att.Name.Contains(":"))
                    {
                        var prefix = att.Name.Split(':')[0];
                        var name = att.Name.Split(':')[1];
                        writer.WriteAttributeString(prefix, name, null, att.Value);
                    }
                    else
